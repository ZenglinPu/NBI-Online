...
import time
import datetime
from NBI.ImageProcess.NBIGenerator import getRandom
import random
...
class User:
    def __init__(self, uid, pwd, registerTime=None, name=None, rank=2, expiresTime=None, workPlace=None, department=None, competent=None):
        self.uid = uid
        self.pwd = pwd
        ...
        # 刚刚注册给予3天高级用户身份
        self.rank = rank
        self.expiresTime = time.mktime(time.strptime((datetime.datetime.now()+datetime.timedelta(days=3)).strftime("%Y-%m-%d 00:00:00"), '%Y-%m-%d %H:%M:%S'))
