...
# 调整图片亮度，对比度，明度，饱和度
def updateImageWithHSV(sourceImage, brightness, contrast, numinosity, satsaturation):
    image = sourceImage
    # 调整亮度
    if not brightness == 0:
        image = updateBrightness(sourceImage, brightness)
    ...
    image = cv2.addWeighted(image, contrast, img2, 1 - contrast, 0)  # addWeighted函数说明如下
